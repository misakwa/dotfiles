...
5. **Verify citations by spot-checking.** You can read the codebase and run `git`/`gh` to verify citations; do not write files, commit, or modify external state. If you're uncertain a cited item exists or says what's claimed, check it. Don't propagate errors.
...
**Explicit gaps.** Things the user asked that the evidence didn't answer. Sources searched that came up empty. Sources that weren't searchable at all, such as an issue tracker not reachable from this environment.
...
- **In-repo docs and tests**: {paths and queries searched}.
- **Other sources** (issue tracker, docs, chat, observability, error tracking): {what was searched}. Or "Not searched. Not reachable from this environment."

### Confidence Summary

One or two sentences summarizing your overall confidence. E.g.:

> "The core rationale (A) is well-supported by direct PR and ticket evidence. The specific threshold value (100) is inferred from the surrounding context but not explicitly documented. The question of whether this was driven by a customer request could not be answered. No relevant in-repo doc content surfaced, and the issue tracker was not reachable."
